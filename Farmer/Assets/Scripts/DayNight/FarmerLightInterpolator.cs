...
namespace Scripts.DayNight
{
    ...
    [ExecuteInEditMode]
    public class FarmerLightInterpolator : MonoBehaviour
    {
        [Serializable]
        ...
        {
            DayNightCycleHandler.RegisterLightBlender(this);
        }

        private void OnDisable()
        {
            DayNightCycleHandler.UnregisterLightBlender(this);
        }
...
#if UNITY_EDITOR
    [CustomEditor(typeof(FarmerLightInterpolator))]
    public class LightInterpolatorEditor : Editor
    {
        private FarmerLightInterpolator m_Interpolator;
        private DayNightCycleHandler m_DayCycleHandler;

        private Slider m_PreviewSlider;

        private void OnEnable()
        {
            m_Interpolator = target as FarmerLightInterpolator;
            m_Interpolator.DisableAllLights();
            m_Interpolator.SetRatio(0);

            m_DayCycleHandler = GameObject.FindObjectOfType<DayNightCycleHandler>();
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            m_PreviewSlider.RegisterValueChangedCallback(evt =>
            {
                ...
                if (m_DayCycleHandler != null)
                {
                    m_PreviewSlider.label = $"Preview time {DayNightService.GetTimeAsString(m_PreviewSlider.value)} ({m_PreviewSlider.value})";
                    m_DayCycleHandler.UpdateLight(m_PreviewSlider.value);
                }
            });
        ...
        {
            var targetLight = serializedObject.FindProperty(nameof(FarmerLightInterpolator.TargetLight));
            var frames = serializedObject.FindProperty(nameof(FarmerLightInterpolator.LightFrames));
            ...
            list.bindItem += (element, i) =>
            {
                ...
                var refLight = entry.FindPropertyRelative(nameof(FarmerLightInterpolator.LightFrame.ReferenceLight));

                element.Q<PropertyField>("ReferenceLightEntry")
                    .BindProperty(refLight);
                element.Q<PropertyField>("TimeEntry")
                    .BindProperty(entry.FindPropertyRelative(nameof(FarmerLightInterpolator.LightFrame.NormalizedTime)));
            };
            ...
            list.selectionChanged += objs =>
            {
                ...
                m_PreviewSlider.value = first.FindPropertyRelative(nameof(FarmerLightInterpolator.LightFrame.NormalizedTime)).floatValue;
            };
            ...
            list.itemsChosen += objects =>
            {
                ...
                var target = first.FindPropertyRelative(nameof(FarmerLightInterpolator.LightFrame.ReferenceLight))
